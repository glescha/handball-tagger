...
export const exportToExcel = async (stats: any, events: AppEvent[], teams: { home: string, away: string }, matchId: string) => {
  ...
  const overviewData = [
    ...
    r("TQekniska Fel", att.turnovers.count, att.totalAttacks, `${att.turnovers.pct}%`),
    r("Tappad Boll", att.lostBalls.count, att.totalAttacks, `${att.lostBalls.pct}%`),
    empty,
    header("PASSNINGAR INNAN MÅL", "Antal", "Totalt", "Procent"),
    r("<2 Pass", att.passes.low.count, att.totalAttacks, `${att.passes.low.pct}%`),
    r("<4 Pass", att.passes.mid.count, att.totalAttacks, `${att.passes.mid.pct}%`),
